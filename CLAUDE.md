# Freesm — 디자인 시스템 워크벤치 · Claude Code 작업 지침

> 제품명은 **Freesm**이다 — Free + 프리즘 발음의 조어(브랜드 미확정 상태로부터 자유로운 시스템, 시드 하나 → 전체 분광).
> v0.58에서 저장소 전체 표기를 Freesm으로 통일했다. 단 `reference/architecture.md`의 `[KDX-pxd] Foundation`은 **실재하는 Figma 원본 파일 이름**이라 그대로 둔다 — 바꾸면 그 파일을 찾을 수 없게 된다. GitHub 저장소·Vercel 프로젝트·로컬 폴더 이름은 계정 소유자만 바꿀 수 있어 별도 조치 대상이다.

기업 홈페이지용 디자인 시스템 워크벤치 **Freesm**이다. ALTO 파운데이션을 승계해
...
- `api/ingest.js` — URL·Figma 소스 해석(서버리스). 결정적·캐시 가능. 스타일시트 수집 범위는 v0.53에서 넓혔다. Figma 토큰은 v0.55부터 **요청 헤더 `X-Figma-Token`(사용자별) → 환경변수 `FIGMA_TOKEN`(배포자)** 순서로 찾는다 — 서버는 토큰을 통과만 시키고 저장·기록하지 않는다. Figma 수집은 v0.56부터 **페이지별 조회 + 면적 가중**이며 응답에 `unit`·`note`를 함께 싣는다. v0.57부터 URL 경로는 이미 받은 HTML에서 **서비스 정체 신호(`site`: 제목·설명·og·JSON-LD 유형·메뉴 링크)** 를 추가 요청 없이 함께 돌려준다.
...
- **제품 내장(1단계)**: 데모 GNB의 **'브랜드 연결'** 모달. 파일(이미지→채도 가중 주요색, 폰트→FontFace 등록)·텍스트(hex/font-family 추출)는 클라이언트에서 즉시 동작. URL·Figma 탭은 `api/ingest.js`(Vercel 서버리스)가 해석 — URL은 HTML+링크된 CSS에서 컬러·폰트 수집(theme-color 가중), Figma는 REST API로 채움색 수집(**모달 안의 토큰 입력칸에 각자의 개인 액세스 토큰**, 비워 두면 배포의 `FIGMA_TOKEN`으로 폴백 — v0.55). 추출 후보 중 시드 선택→커스텀 브랜드 파이프라인으로 연결.
...
- **사용자별 Figma 토큰(v0.55)**: v0.54까지 Figma 탭은 배포에 심어 둔 환경변수 `FIGMA_TOKEN` 하나로만 동작했다. 그 구조에는 두 가지 결함이 있다 — ① 방문자가 누구든 **배포자의 자격으로** Figma에 들어가므로 자기 파일은 열지 못하고(403), ② 인증 없는 엔드포인트가 배포자가 볼 수 있는 모든 파일을 대신 읽어 준다. 토큰 출처를 **요청 헤더 `X-Figma-Token`(사용자별) → `process.env.FIGMA_TOKEN`(배포자 폴백)** 순서로 바꿔, 각자 자기 권한으로 자기 파일만 읽게 했다. 설계상 정한 것들: **쿼리스트링이 아니라 헤더** — URL은 접근 로그·리퍼러·브라우저 기록에 그대로 남는다. 커스텀 헤더는 **CORS 프리플라이트**를 부르므로 핸들러가 `OPTIONS`에 204로 답하고 `Access-Control-Allow-Headers: X-Figma-Token`을 알린다. 토큰은 그대로 아웃바운드 헤더 값이 되므로 **출력 가능한 ASCII만 통과**시킨다(`/^[\x21-\x7e]+$/`) — CRLF가 섞이면 헤더가 갈라진다. 값이 잘못됐을 때 서버 토큰으로 **몰래 폴백하지 않는다**(남의 자격으로 읽히면 안 된다). 토큰이 어디에도 없으면 `need:'token'`을 함께 돌려주고, 화면은 문장을 띄우는 대신 **커서를 토큰 칸으로 옮긴다** — 이전 버전에서 사용자가 토큰을 '파일 링크' 칸에 붙여넣으려 한 게 정확히 이 실패 모드였다. 링크 파싱도 넓혔다: `file`·`design`·`board`·`proto`·`slides`·`deck` 경로에서 key를 뽑고(10자 하한으로 짧은 경로 조각 오인 방지), 링크가 아니면 앞쪽 영숫자를 key로 본다. 화면 쪽 토큰 입력칸은 `type="password"`·`autocomplete="off"`이며 **메모리에만** 있다 — 저장소·쿠키에 쓰지 않으므로 새로고침하면 사라진다. 검증은 `verify55.js` 37건(토큰 출처 순서·주입 방어·프리플라이트·오류 문구에 토큰 미노출·헤더 전송·저장 흔적 없음·포커스 이동).
- **수집 단위를 '면적'으로(v0.56 — 전 경로 진단 1단계)**: "코레일톡 파일을 연결했는데 점유율도 낮은 레드를 Primary로 잡는다"는 보고를 계기로 네 수집 경로(URL·Figma·북마클릿·이미지)를 각각 실측했고, **판별이 아니라 계량**이 틀렸다는 결론이 나왔다. `igScore`(빈도√ × 채도 × 명도 적합 × 대비 적합)는 올바른 입력에서는 올바른 답을 낸다 — 문제는 그 앞이다. **네 경로 중 셋이 '사람이 보는 단위(면적)'가 아니라 '소스 파일이 저장하는 단위(선언 횟수·노드 개수·요소 개수)'를 세고 있었다.** 픽셀을 세는 `imgDominant`만 단위가 맞았고, 그건 입력이 이미 렌더된 상태로 도착하기 때문이다. 주석 번호·강조 박스·화살표처럼 **작지만 개수가 많은 것**이 전면 배경·CTA·탭바처럼 **크지만 개수가 적은 것**을 이기는 구조였다. 1단계는 Figma와 북마클릿 두 경로의 단위를 면적으로 바꾼다.
  **① Figma — 페이지별로 따로 받는다.** 기존 `GET /v1/files/:key?depth=4`는 한 번에 트리를 받는 대신 **4단에서 잘렸다** — 컴포넌트 인스턴스 안쪽(대개 5~8단)의 색은 애초에 응답에 없었다. 이제 `depth=1`로 **페이지 목록만** 받고, 페이지마다 `GET /v1/files/:key/nodes?ids=<pageId>`로 **depth 제한 없이** 서브트리를 받는다(응답 하나가 비대해지지 않으면서 깊이 상한이 사라진다). 상한은 **페이지 10개·전체 9초·페이지당 8초**.
  **② 면적 가중 · 숨김·투명 반영.** 노드의 `absoluteBoundingBox` 넓이 × 채움 불투명도를 색에 더한다. `visible:false`는 **서브트리째** 제외하고(숨긴 페이지는 애초에 요청하지 않는다), 실효 불투명도 5% 이하도 버린다. **부모와 자식이 같은 색·같은 사각형이면 한 번만 센다** — 안 그러면 중첩 깊이가 곧 가중치가 된다. 화면의 0.1% 미만(주석 점·1px 선)은 후보에서 뺀다.
  **③ 천분율로 환산한다.** `counts`는 이제 '개수'가 아니라 **면적 점유 ‰**다. `igScore`의 빈도항이 `√`라 단조이므로 순위 의미는 보존되고, `/api/judge`에 넘어가는 값도 개수보다 나은 입력이 된다(판정 층 자체는 3단계 범위).
  **④ 단위와 누락을 응답에 싣는다.** `/api/ingest`가 `unit:'area'|'count'`와 `note`(상한에 걸려 못 읽은 페이지 안내)를 함께 돌려주고, 화면은 근거 문장을 `Figma 실측 — 보이는 레이어의 면적 점유율`로 바꾸고 `note`를 그 뒤에 **그대로 붙인다**. 상한 때문에 일부만 읽었다는 사실을 조용히 넘기지 않는다.
  **⑤ 북마클릿 bv4.** 기존 코드는 `getBoundingClientRect()`를 부르고도 **넓이를 버린 채 요소 하나를 1로 셌다**. 이제 배경은 면적, 텍스트 색은 면적×0.18(직접 텍스트 노드가 있을 때만), 보더는 둘레×두께로 더하고, **부모와 배경색이 같으면 배경을 다시 세지 않는다**. computed style은 `Map`으로 캐시한다. 출력은 `{hex, count:‰, n:요소 수, area:원면적}` + `unit:'area'`. 구버전(`bv<4`) 결과에는 "면적이 아니라 요소 개수로 세기 때문에 1순위가 달라질 수 있다"는 경고를 띄운다.
  **실측 전후**(같은 모의 코레일톡 파일·같은 렌더 화면): Figma는 `#e8342c`(주석 빨강) 78.6% 1위 → **`#0f6fc6` 642‰(igScore 19.88 vs 빨강 5.95)** 로 뒤집혔고, 북마클릿은 `#e8342c` 24개·88.9% 1위 → **`#0f6fc6` 877‰** 로 실제 면적 점유율 87.7%와 일치했다. 검증은 `verify56.js` 34건(서버 19: 면적>개수·천분율 합 1000·7단 깊이 도달·본문 요청에 depth 없음·숨긴 페이지 미요청·불투명도·중첩 중복·0.1% 절사·note 문구·빈 파일·전체 실패·정렬·URL 경로는 여전히 count / 화면 15: bv4·실브라우저 실행·큰 면 vs 작은 뱃지·근거 문구·구버전 경고·note 노출). **URL 경로의 단위는 아직 `count`다** — 선언 티어 확보율과 함께 2단계에서 다룬다. 진단 전문은 `Freesm-브랜드분석-전경로-진단-v0.6.md`(Claude 프로젝트).
- **도메인에 맞는 예시 화면 자동 생성(v0.57)**: "도메인을 넣었을 때 해당 서비스를 분석하고 예시 페이지를 그 도메인에 맞게 추천해 달라"는 요청. 색만 가져오면 **무슨 브랜드인가**는 알아도 **무슨 서비스인가**는 모르는 상태였고, 예시 화면 11종은 조각투자 서비스를 가정한 고정 문구를 쓰고 있었다. 이제 URL을 연결하면 색을 적용한 뒤 **화면 3개가 자동으로 세워진다**(선택 층이 아니라 기본 동작).
  **① 신호는 이미 받아 둔 HTML에서만 뽑는다 — 추가 요청 0회.** `ingestUrl`이 색을 위해 이미 읽은 초기 HTML에서 `title`·`description`·`og:site_name`·`og:type`·`lang`·JSON-LD `@type`, 그리고 **GNB·푸터 링크**를 추출해 `site`로 함께 싣는다. 링크가 가장 강한 근거다 — 그건 그 서비스가 **스스로 밝힌 정보구조**이고, 요금·약관·공지·문의 화면이 있는 서비스인지가 거기서 드러난다. 본문 링크는 콘텐츠지 메뉴가 아니므로 `<nav>`/`<header>`/`<footer>` 안쪽만 보고, 그 영역이 없는(JS로 그리는) 문서에서만 전체 `<a>`로 후퇴한다. 상한 28개, 텍스트 40자, `href`는 경로만 남긴다.
  **② 규칙 층이 기본, 에이전트 층이 선택.** 9개 표(요금·약관·공지·이벤트·문의·상품·상세·마이·소개)의 정규식으로 신호를 세어 상위 3개를 고르고, 동점이면 표 순서가 우선순위가 된다(무작위로 흔들리지 않게 인덱스를 보조키로 쓴다). 근거가 하나도 없으면 어떤 서비스에나 있는 **소개·상품·문의**를 세운다. **키가 없어도·`file:`에서도·네트워크가 죽어도 화면 3개는 나온다.**
  **③ 에이전트 계약은 v0.51 그대로.** `/api/judge`에 `mode:'screens'`를 더했다 — 에이전트는 화면을 새로 만들지 않고 **`SCR_TPL`의 인덱스만** 고른다. 범위 밖·중복 인덱스는 그 항목만 버리고, 하나도 못 건지면 전체를 버려 규칙 층으로 내려간다. 실패는 전부 200·무음이다.
  **④ 남의 상표는 데모에 심지 않는다.** 제목·설명·경로는 문장으로 받되 **브랜드 토큰이 섞이면 그 필드를 통째로 버린다**(클라이언트가 템플릿 기본 문구로 되돌아간다). 토큰은 호스트 라벨(`www.instagram.com` → `instagram`)**과** 사이트가 스스로 밝힌 이름(`og:site_name`) 양쪽에서 뽑는다 — 에이전트는 한국어로 답하고 '인스타그램'에는 `instagram`이 없기 때문이다.
  **⑤ 되돌릴 수 있고 쌓이지 않는다.** 자동 생성한 화면 id를 따로 들고 있어 **다시 분석하면 갈아끼우고**(6개로 쌓이지 않는다), '생성한 화면 지우기'로 한 번에 지운다. 보고 있던 화면이 지워지면 홈으로 돌려놓는다. 카드는 **복제가 아니라 26개 섹션 어휘 안에서의 근사치**임을 명시한다. Figma 경로에서는 만들지 않는다 — 파일에는 '이 서비스의 메뉴'가 없다. 검증은 `verify57.js` 45건(ingest 11 · judge 16 · 화면 18).
- **로드맵(v0.51 2·3단계, 미구현)**: 2단계 — 로고 이미지를 함께 보내는 **비전 판정**(로고 이미지 외부 전송 고지 문구 결정 필요). 3단계 — 그라디언트 채택·형태 모드·서체까지 판정 결과를 자동 적용. 설계 근거 전문은 `Freesm-브랜드연결-에이전트판정-설계-v0.51.md`(Claude 프로젝트).

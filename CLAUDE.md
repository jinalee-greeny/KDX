...
- `api/ingest.js` — URL·Figma 소스 해석(서버리스). 결정적·캐시 가능. 스타일시트 수집 범위는 v0.53에서 넓혔다. Figma 토큰은 v0.55부터 **요청 헤더 `X-Figma-Token`(사용자별) → 환경변수 `FIGMA_TOKEN`(배포자)** 순서로 찾는다 — 서버는 토큰을 통과만 시키고 저장·기록하지 않는다. Figma 수집은 v0.56부터 **페이지별 조회 + 면적 가중**이며 응답에 `unit`·`note`를 함께 싣는다.

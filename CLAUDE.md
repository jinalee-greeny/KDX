...
2. **램프 생성·적용 (데모 내장)**: 데모 인스펙터 Brand 그룹의 **커스텀 패널**에 시드 hex(+폰트) 입력 → HSL 기반으로 05~70 램프 자동 생성(수제 램프의 명도 곡선 근사: 05·10·20 라이트 / 40 비비드 / 55·60·70 다크) → `[data-brand="custom"]`로 주입, 4번째 브랜드로 전체 캐스케이드. p50의 흰 텍스트 대비(AA 4.5) 자동 검사. 비교 매트릭스에도 4번째 열로 합류.
